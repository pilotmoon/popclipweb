...
| `app`                 | Dictionary           | Dictionary describing a "target" app or website that this action sends text to or otherwise interacts with. You can, optionally, specify that the app must be present on the system; if not present, PopClip will prompt the user to install. This field is also used by the PopClip Extensions Directory to link an app name in the description to the app website. See [The `app` dictionary](#the-app-dictionary).                                                                                               |
...
| `wantsPrimaryDisplay` | Boolean              | If `true`, the action asks to be the popup's primary button — the one centred above the pointer when the popup appears. If more than one visible button asks, the leftmost wins. Default is `false`. (Used by the built-in Copy and Paste actions.)                                                                                                                                                                                                                                                                 |
...
When shown in the PopClip Extensions Directory, instances of the app name in the extension's description will be linked to the app's website.
