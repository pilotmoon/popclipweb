...
| `entitlements`              | Array                | Only applies to JavaScript extensions. The possible values are `network` (allows use of XMLHttpRequest), `dynamic` (allows dynamically generated actions) and `script` (allows [calling external scripts](./external-scripts)). The `dynamic` entitlement cannot be combined with `network` or `script`. |
...
| `shellScriptRationale`    | String               | A brief explanation of why the extension needs a [Shell Script action](./shell-script-actions) instead of JavaScript. Not used by the app; required when [submitting](/extensions/submit#shell-script-policy) an extension with a Shell Script action to the directory.                   |
